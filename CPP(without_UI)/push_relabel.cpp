...
int main()
{
    ...
    ifstream fin("/Volumes/Transcend/pushhard/pushhard/demo.txt");
    string s;
    ...
    for(int i = 0;i<Gmmpre.size()/2;i++){
        g.add(p, i+1, (int)(1000*Gmmpre[2*i]));
        g.add(i+1, t, 1000-(int)(1000*Gmmpre[2*i]));
    }
    //    for(int i = 0;i<g.edge.size();i++){
    //        cout<<g.edge[i].u<<"_"<<g.edge[i].v<<"_"<<g.edge[i].flow<<"_"<<g.edge[i].capacity<<"_"<<endl;
    //    }
    g.mincut();
    vector<int> result = {};
    for(int i=1;i<g.V;i++){
